...
namespace net.r_eg.vsCE.UI
{
    public class Util
    {
        ...
        public static void setDescriptionHeightFor(PropertyGrid grid, int height)
        {
            foreach(Control c in grid.Controls)
            {
                ...
                FieldInfo inf = c.GetType().BaseType.GetField("userSized", BindingFlags.Instance | BindingFlags.NonPublic);
                inf.SetValue(c, true);
                return;
            }
        }
