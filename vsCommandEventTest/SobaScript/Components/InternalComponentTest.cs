...
namespace net.r_eg.vsCE.Test.SobaScript.Components
{
    [TestClass]
    public class InternalComponentTest
    {
        ...
        {
            InternalComponent target = new InternalComponent(new Soba(), new StubEnv());
            target.Eval("#[vsCE events.Type.item(1)]");
        }
        ...
        {
            InternalComponent target = new InternalComponent(new Soba(), new StubEnv());
            target.Eval("[vsCE events.Type.item(name)]");
        }
        ...
        {
            InternalComponent target = new InternalComponent(new Soba(), new StubEnv());
            target.Eval("[vsCE events.Type.item(1).test]");
        }
        ...
        public void pEnabledParseTest1()
        {
            ...
            Assert.AreEqual(Value.From(true), target.Eval("[vsCE events.Pre.item(1).Enabled]"));
            Assert.AreEqual(Value.From(true), target.Eval("[vsCE events.Pre.item(\"Name1\").Enabled]"));
            Assert.AreEqual(Value.From(false), target.Eval("[vsCE events.Pre.item(2).Enabled]"));
            Assert.AreEqual(Value.From(false), target.Eval("[vsCE events.Pre.item(\"Name2\").Enabled]"));
        }
        ...
        {
            InternalComponentAccessor target = new InternalComponentAccessor();
            target.Eval("[vsCE events.Pre.item(1).Enabled = 1true]");
        }
        ...
        public void pEnabledParseTest3()
        {
            var target = new InternalComponentAccessor();
            Assert.AreEqual(Value.From(true), target.Eval("[vsCE events.Pre.item(1).Enabled]"));
            Assert.AreEqual(Value.Empty, target.Eval("[vsCE events.Pre.item(1).Enabled = false]"));
            Assert.AreEqual(Value.From(false), target.Eval("[vsCE events.Pre.item(1).Enabled]"));

            Assert.AreEqual(Value.From(false), target.Eval("[vsCE events.Pre.item(\"Name2\").Enabled]"));
            Assert.AreEqual(Value.Empty, target.Eval("[vsCE events.Pre.item(\"Name2\").Enabled = true]"));
            Assert.AreEqual(Value.From(true), target.Eval("[vsCE events.Pre.item(\"Name2\").Enabled]"));
        }
        ...
        {
            InternalComponentAccessor target = new InternalComponentAccessor();
            target.Eval("[vsCE events.Pre.item(1).Status.Has Errors]");
        }
        ...
        {
            InternalComponentAccessor target = new InternalComponentAccessor();
            Assert.AreEqual("false", target.Eval("[vsCE events.Pre.item(1).Status.HasErrors]"));
        }
        ...
        {
            InternalComponentAccessor target = new InternalComponentAccessor();
            target.Eval("[vsCE events.Pre.item(1).Status.NotExistProp]");
        }
        ...
        {
            var target = new InternalComponent(new Soba(), new StubEnv());
