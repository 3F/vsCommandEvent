...
namespace net.r_eg.vsCE.Test.SobaScript.Components
{
    public class InternalComponentTest
    {
        ...
        [InlineData("[vsCE events.Pre.item(1).Status.Has Errors]")]
        [InlineData("[vsCE events.Pre.item(1).Status.NotExistProp]")]
        ...
        [Theory]
        [InlineData("[vsCE events.Type.item(name)]")]
        [InlineData("[vsCE events.Type.item(1).test]")]
        ...
        [Theory]
        [InlineData("#[vsCE events.Type.item(1)]")]
        [InlineData("vsCE events.Type.item(1)")]
        [InlineData("[vsCE events.Pre.item(1).Enabled = 1true]")]
        ...
        {
            InternalComponentAccessor target = new();
            Assert.Equal("false", target.Eval("[vsCE events.Pre.item(1).Status.HasErrors]"));
        }
        ...
        {
            InternalComponentAccessor target = new();
            Assert.Equal(Value.From(true), target.Eval("[vsCE events.Pre.item(1).Enabled]"));
            Assert.Equal(Value.From(true), target.Eval("[vsCE events.Pre.item(\"Name1\").Enabled]"));
            Assert.Equal(Value.From(false), target.Eval("[vsCE events.Pre.item(2).Enabled]"));
            Assert.Equal(Value.From(false), target.Eval("[vsCE events.Pre.item(\"Name2\").Enabled]"));
        }
        ...
        public void pEnabledParseTest3()
        {
            InternalComponentAccessor target = new();
            Assert.Equal(Value.From(true), target.Eval("[vsCE events.Pre.item(1).Enabled]"));
            Assert.Equal(Value.Empty, target.Eval("[vsCE events.Pre.item(1).Enabled = false]"));
            Assert.Equal(Value.From(false), target.Eval("[vsCE events.Pre.item(1).Enabled]"));

            Assert.Equal(Value.From(false), target.Eval("[vsCE events.Pre.item(\"Name2\").Enabled]"));
            Assert.Equal(Value.Empty, target.Eval("[vsCE events.Pre.item(\"Name2\").Enabled = true]"));
            Assert.Equal(Value.From(true), target.Eval("[vsCE events.Pre.item(\"Name2\").Enabled]"));
        }
